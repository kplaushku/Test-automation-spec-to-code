...
> You have Specto installed and want to upgrade to the latest version to get new features, bug fixes, or updated slash commands. This guide covers both upgrading the CLI tool and updating your project files.
...
`uvx` runs a temporary copy of Specto for that single command. It does not update a persistent `specify` installed with `uv tool install`, `pipx`, or another tool manager. If a newer feature works through `uvx` but your local `specify` still reports an older version, upgrade the persistent CLI with the command that matches your install method.
...
When Specto releases new features (like new slash commands or updated templates), you need to refresh your project's Specto files.
...
Projects that do not use Git can still work with Specto by setting `SPECIFY_FEATURE_DIRECTORY` to the feature directory path before planning commands:
...
3. **Some files may be overwritten** - If you already have Specto files (`.claude/`, `.specify/`, etc.), they'll be replaced with the new versions

**What gets overwritten:**

Only Specto infrastructure files:
...
- Any other files not part of Specto templates
...
- ✅ **Expected** when upgrading an existing Specto project
- ✅ **Expected** when adding Specto to an existing codebase
...
- **Initial setup:** `specify init` to bootstrap Specto in your project
...
Specto follows semantic versioning for major releases. The CLI and project files are designed to be compatible within the same major version.
