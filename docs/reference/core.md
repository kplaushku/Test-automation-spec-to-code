...
Creates a new Specto project with the necessary directory structure, templates, scripts, and AI coding agent integration files.
...
This command stays offline. If a command behaves like an older Specto version or an expected CLI feature is missing, run `specify self check` to check whether your local CLI is behind the latest release.
...
Displays the Specto CLI version, Python version, platform, and architecture.
