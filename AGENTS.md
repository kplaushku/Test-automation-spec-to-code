...
## About Specto and Specify

**Specto** is a spec-driven **test automation** toolkit. The same
spec-driven workflow used to build software is repurposed to generate executable
tests: you write requirements in natural language, and Specto produces
runnable test suites in the framework chosen at planning time (Robot Framework,
Playwright, Cypress, Selenium, or Appium). Neutral test scenarios go in;
framework-specific tests come out. It is derived from
[github/spec-kit](https://github.com/github/spec-kit) (MIT) and is an
...
Specto framework. It sets up the directory structures, templates,
presets/extensions, and AI agent integrations that support the workflow.

The toolkit supports multiple AI coding assistants, allowing teams to use their
...
Set `context_file` on the integration class. The base integration setup creates or updates the managed Specto section in that file, and uninstall removes the managed section when appropriate.
...
# Delimiters for the managed Specto section
context_markers:
  start: "<!-- SPECKIT START -->"
  end: "<!-- SPECKIT END -->"
```
...
5. Sets `context_file = "AGENTS.md"` so the base setup manages the Specto context section there
